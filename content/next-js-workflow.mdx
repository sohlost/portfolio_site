...
# Understanding Next.js: A Simple Guide to the Popular React Framework
...
## When to Use Different Rendering Methods

Here's a simple guide for choosing the right rendering method:

```mermaid
flowchart TD
    A[New Page] --> B{Does data change often?}
    B -->|No| C[Use Static Generation]
    B -->|Yes| D{Need SEO?}
    D -->|Yes| E[Use SSR]
    D -->|No| F[Use Client-side]
    C --> G[getStaticProps]
    E --> H[getServerSideProps]
    F --> I[useEffect + fetch]
